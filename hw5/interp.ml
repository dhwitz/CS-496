...
and
  ...
  | Case(cond,branches) -> 
      let e1 = eval_expr en cond
      in find_branch en e1 branches
  | Debug ->
...
and
  find_branch : Ds.env -> Ds.exp_val -> Ast.branch list -> Ds.exp_val = fun en e1 branch_list ->
    match branch_list with
    | [] -> failwith "Could not find correct branch"
    | Branch(str,args_list,expr)::xs -> 
      (match e1 with
        | TaggedVariantVal(string, expr_list) -> 
          if string = str
          then eval_expr (map_branch en args_list expr_list) expr
          else find_branch en e1 xs
        | _ -> failwith "Must create a case with a user defined type")
and
  map_branch : Ds.env -> string list -> Ds.exp_val list -> Ds.env = fun en args_list expr_list ->
    match (args_list, expr_list) with
    | ([], []) -> en
    | ([], _) -> failwith "This user defined type takes more arguments than provided"
    | (_, []) -> failwith "This user defined type takes less arguments than provided"
    | (x::xs, y::ys) -> 
      map_branch (extend_env en x y) xs ys
